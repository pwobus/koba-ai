...
* It configures the chat model using the ChatOpenAI class, which relies on OpenAI's "gpt-4o" chat-completions model by default (or "gpt-4o-mini" if you set `OPENAI_MODEL` accordingly) when the environment variables have not been configured previously.
