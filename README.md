...
* It configures the chat model using the ChatOpenAI class, which relies on OpenAI's "gpt-4o" chat-completions model by default (or "gpt-4o-mini" if you set `OPENAI_MODEL` accordingly) when the environment variables have not been configured previously. Realtime-preview models are not supported by this client, so any `OPENAI_MODEL` value containing `realtime` will fall back to `gpt-4o` to avoid `model_not_found` errors.
...
```bash
# OPENAI
OPENAI_MODEL=<YOUR_GPT_MODEL> # defaults to gpt-4o (set to gpt-4o-mini if preferred)
