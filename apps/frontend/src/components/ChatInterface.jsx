...
export const ChatInterface = ({ hidden, ...props }) => {
  const { loading, message, startRecording, stopRecording, recording } = useSpeech();
  if (hidden) {
    return null;
  }
    ...
    <div className="fixed top-0 left-0 right-0 bottom-0 z-10 flex justify-between p-4 flex-col pointer-events-none">
      <div className="self-start backdrop-blur-md bg-white bg-opacity-50 p-4 rounded-lg">
        ...
        <p className="text-gray-600">
          {loading
            ? "Loading..."
            : "Use the button below to start or end a real-time conversation."}
        </p>
      </div>
      ...
      <div className="flex items-center gap-2 pointer-events-auto max-w-screen-sm w-full mx-auto">
        ...
        <button
          ...
          onClick={recording ? stopRecording : startRecording}
          className={`flex-1 bg-gray-500 hover:bg-gray-600 text-white p-4 px-10 font-semibold uppercase rounded-md ${
            recording ? "bg-red-500 hover:bg-red-600" : ""
          } ${loading || message ? "cursor-not-allowed opacity-30" : ""}`}
        >
